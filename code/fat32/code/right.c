...
fat32_fs_t fat32_mk(mbr_partition_ent_t *partition) {
  ...
  //unimplemented();
  struct fsinfo * info = pi_sec_read(partition->lba_start + 1, 1);
  ...
  unsigned n_entries = boot_sector.nsec_per_fat * NDIR_PER_SEC; // from the boot sector
  // unimplemented();
   ...
   */
  uint32_t *fat = pi_sec_read(fat_begin_lba, sec_per_cluster);
  // unimplemented();
...
static void read_cluster_chain(fat32_fs_t *fs, uint32_t start_cluster, uint8_t *data) {
  ...
  uint32_t cnt = 0;
  do {
    pi_sd_read(data + cnt, cluster_to_lba(fs, start_cluster), fs->sectors_per_cluster);
    start_cluster = fs->fat[start_cluster];
    cnt += (NBYTES_PER_SECTOR * fs->sectors_per_cluster);
  } while (fat32_fat_entry_type(start_cluster) != LAST_CLUSTER);
...
static fat32_dirent_t *get_dirents(fat32_fs_t *fs, uint32_t cluster_start, uint32_t *dir_n) {
  ...
  //unimplemented();
  uint32_t len = get_cluster_chain_length(fs, cluster_start);
  *dir_n = len * fs->sectors_per_cluster * 16;

  // allocate a buffer large enough to hold the whole directory
  //unimplemented();
  fat32_dirent_t * buf = kmalloc(len * fs->sectors_per_cluster * NBYTES_PER_SECTOR);

  // read in the whole directory (see `read_cluster_chain`)
  // unimplemented();
  read_cluster_chain(fs, cluster_start, (uint8_t *)buf);
...
pi_dirent_t *fat32_stat(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  fat32_dirent_t * dirents = get_dirents(fs, directory->cluster_id, &num_dirents);
  ...
  int match = find_dirent_with_name(dirents, num_dirents, filename);
  ...
  *dirent = dirent_convert(&dirents[match]);
  return dirent;
}
...
pi_file_t *fat32_read(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  // unimplemented();
  pi_dirent_t * match = fat32_stat(fs, directory, filename) ;

  // figure out the length of the cluster chain
  // unimplemented();
  uint32_t len = get_cluster_chain_length(fs, directory->cluster_id);
  printk("length = %d\n",len);

  // allocate a buffer large enough to hold the whole file
  // unimplemented();
  uint8_t * buf = kmalloc(match->nbytes);

  // read in the whole file (if it's not empty)
  // unimplemented();
  read_cluster_chain(fs, match->cluster_id, buf);

  pi_file_t *file = kmalloc(sizeof(pi_file_t));
  *file = (pi_file_t) {
    .data = buf,
    .n_data = match->nbytes,
    .n_alloc = match->nbytes,
  };
  return file;
}
