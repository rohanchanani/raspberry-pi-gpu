...
void notmain(void)
{
	...
	for (int i=0; i<NUM_QPUS; i++) {
	    ...
	    gpu->unif[i][1] = GPU_BASE + (uint32_t)&gpu->A + i*N*4 / NUM_QPUS;
	    gpu->unif[i][2] = GPU_BASE + (uint32_t)&gpu->B + i*N*4 / NUM_QPUS;
	    gpu->unif[i][3] = GPU_BASE + (uint32_t)&gpu->C + i*N*4 / NUM_QPUS;
	    gpu->unif[i][4] = i;
